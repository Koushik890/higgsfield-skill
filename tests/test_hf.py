...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        self.spec = hf.load_model("seedance-2.5-t2v")
        self.api = FakeAPI()
...
class PricingTests(Base):
    def test_seedance_price_from_description(self):
        self.assertEqual(hf.video_token_price(self.spec, {"prompt": "x", "duration": 5}, PRICING_TEXT), Decimal("2.3112"))
        self.assertEqual(hf.video_token_price(self.spec, {"prompt": "x", "duration": 4}, PRICING_TEXT), Decimal("1.8490"))
        self.assertEqual(hf.video_token_price(self.spec, {"prompt": "x", "duration": 10, "resolution": "480p",
                                                          "aspect_ratio": "9:16"}, PRICING_TEXT), Decimal("0.6500"))
...
class JobTests(Base):
    ...
    def test_attach_recovers_lost_job(self):
        job = hf.attach(self.store, "recovered", RID.upper(), model="seedance-2.5-t2v")
        self.assertEqual(job["request_id"], RID)
        with self.assertRaises(hf.HFError):
...
class CliTests(unittest.TestCase):
    def test_validate_command(self):
        code = hf.main(["validate", "--model", "seedance-2.5-t2v", "--json", json.dumps(REQUEST)])
